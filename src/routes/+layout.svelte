...
<div class="container w-[75vw] mx-auto flex flex-col space-y-4">
  <header>
    <div class="flex h-16 border-b border-b-slate-200 py-4">
        ...
        <ul class="flex">
          {#each navigation as item}
            <li>
              <a
                href={item.href}
                class="hover:text-gray-700 text-gray-400 cursor-pointer p-5 {$page
      ...
      <nav-right class="self-center flex justify-end w-full mr-4 space-x-12">
        <label
          for="file-upload"
          class="btn {!($isMetamaskInstalled && $user?.token)
            ? 'btn-disabled'
            : ''}">Upload PDF</label
        >
        {#if $user?.token}
          ...
          <form
            ...
            use:enhance={async (event) => {
              ...
              await $ethereum
                ...
                .then(() =>
                  event.data.set("user", $ethereum?.selectedAddress ?? "")
                )
                .catch(() => {
                  event.cancel();
                });
  ...
  <footer class="m-auto p-4 text-gray-400">
    <div>
      Powered by OpenAI, Pinecone and the <i
